...
imwrite(bw,['IBW.tif'],'tif')
%figure;imshow(bw)
% fill holes within circular particles
[centres,radii] = imfindcircles(bw,[20 70],"ObjectPolarity","bright","Sensitivity",0.9); % change radii range here
...
[bcentres,bradii] = imfindcircles(bw,[14,20],"ObjectPolarity","dark","Sensitivity",0.95);
bradii

for kk=1:size(bcentres,1)
    bw = drawCircle(bw, bcentres(kk,:), 40*0.8);
end
